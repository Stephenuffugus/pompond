/* ============================================================
  ...
  function speciesDetail(kid,arch){
    ...
    const cells=all.map(c=>{
      ...
      const tags=[]; if(c.variant&&c.variant!=="classic")tags.push("🎨"+esc(CritterEngine.variantName(c.variant)));
      if(sh)tags.push("✨"); if((c.tier||0)>0)tags.push("🌟"+esc(Evolution.tierName(c.tier))); tags.push(CritterEngine.rarityName(c.rarity));
      return `<div class="sdcell${c.fused?" fused":""}" data-id="${c.id}"><div class="sdart">${renderCritter(c.seed,c.archetype,c.rarity,{tier:c.tier,shiny:c.shiny,variant:c.variant})}</div><div class="sdtags">${tags.join(" · ")}</div>${c.fused?'<span class="sdfused">🧬 fused</span>':""}</div>`;
    }).join("");
