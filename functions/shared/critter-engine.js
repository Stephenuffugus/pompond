...
(function (root, factory) {
  ...
  const ARCH={
    ...
    snail(t){const g=bodyGrad(t);return [`<defs>${g.def}</defs>${shadow(40,84,24)}<ellipse cx="26" cy="76" rx="18" ry="8" fill="${t.bodyL}"/><path d="M16 71 v-13 M22 71 v-15" stroke="${t.bodyL}" stroke-width="3" stroke-linecap="round"/><circle cx="16" cy="56" r="2" fill="${t.acc}"/><circle cx="22" cy="55" r="2" fill="${t.acc}"/><circle cx="54" cy="56" r="17" fill="${t.bodyD}"/><circle cx="54" cy="56" r="12" fill="${g.fill}"/><circle cx="54" cy="56" r="6" fill="${t.bodyD}"/><circle cx="16" cy="72" r="2" fill="#2c2622"/><circle cx="22" cy="72" r="2" fill="#2c2622"/>${blush(t,12,30,74)}`,54,40];},
    ...
    seahorse(t){const g=bodyGrad(t);return [`<defs>${g.def}</defs>${shadow(50,88,16)}<path d="M50 30 q14 2 12 18 q-2 16 -14 18 q-12 2 -10 -10 q4 6 10 4 q8 -4 6 -16 q-2 -10 -10 -12z" fill="${g.fill}"/><path d="M40 38 q-7 0 -9 8 q5 -2 8 -3z" fill="${t.acc}"/><path d="M52 30 q4 -5 9 -4 q-2 5 -7 6z" fill="${t.acc}"/><circle cx="55" cy="36" r="3.4" fill="#2c2622"/><circle cx="54" cy="35" r="1.1" fill="#fff"/>${blush(t,50,62,42)}`,50,26];},
  ...
  function render(seed,archetype,rarity){
    ...
    let aura='', spark='';
    if(rarity===1){
      aura=`<circle cx="50" cy="56" r="47" fill="hsl(${t.hue2},80%,62%,.15)"/>`;
    }else if(rarity===2){
      const gid='ar'+c;
